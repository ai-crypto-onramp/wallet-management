...
test:
	go test ./... -timeout 120s -coverprofile=coverage.out -coverpkg=./...

test-race:
	go test ./... -race -timeout 180s -coverprofile=coverage.out -coverpkg=./...

test-integration:
	go test ./... -race -tags=integration -timeout 300s -coverprofile=coverage.out -coverpkg=./...
